...
Running the queries once, there does not seem to be any significant difference between the learning outcomes, except that $E_mek$ alone is not enough to efficiently hit the ball, if useless swings are counted.
...
Observing only $E_mek$ Leads to 146 swings and 44 hits when trying to minimize swings. Minimizing for hits seems to be the same, with 41 hits.
